...
IRU #8 looks like an outlier by relative abundance too.
...
```
IRU #8 also looks like an outlier by total counts, but interestingly it has **more counts** than any of the other right lung samples.  It's not clear what is going on with IRU #8, so the simplest thing to do is exclude it and move on . . .
...
#### Permanova
There does seem to be some weak clustering of samples in the ordination plot. A permanova analysis will allows us to quantify this. 
...
#### LefSE
```{r right1_with_SRU_generate_lefse, echo=FALSE, results='hide', eval=FALSE}
...
### Right Lung, no Antibiotics, no Saline, excluding IRU #8 (WRU, IRU)
...
#### Permanova
The ordination plot does not seem to show any treatment differences, but we will check with a permanova analysis. 
...
Despite the lack of significance from the permanova, LefSE does find once significant taxa.  This should be regarded with skepticism!
...
print(right_lung2_WRU.nmds.plot)
```

### Permanova
The ordination plot, suggest some differences between the two cohorts of the whole gastric fluid aspiration treatment. A permanova analysis allows us to quantify this. 
...
#### WLU vs WRU, group2
```{r, echo=FALSE, results = 'asis'}
right_left3_WLU_WRU_group2.ps = subset_samples(bacteria.ps,
                               (group %in% c("WLU","WRU"))
                               & (animal >= 13))

right_left3_WLU_WRU_group2.pruned.ps = PruneTaxa(right_left3_WLU_WRU_group2.ps)

## Transform to even sampling depth.
right_left3_WLU_WRU_group2.even.ps = TransformCounts(right_left3_WLU_WRU_group2.pruned.ps)
group_var = "group"
```
```{r, echo=FALSE, results='hide'}
sample_data(right_left3_WLU_WRU_group2.ps)
```

```{r, echo=FALSE, results='hide'}
right_left3_WLU_WRU_group2.nmds.plot = NMDSPlot(right_left3_WLU_WRU_group2.even.ps, grouping = group_var)
print(right_left3_WLU_WRU_group2.nmds.plot)
```
...
```{r, echo=FALSE}
right_left3_WLU_WRU_group2.perm = RunPermanova(right_left3_WLU_WRU_group2.even.ps, group_var)
right_left3_WLU_WRU_group2.adonis = right_left3_WLU_WRU_group2.perm$adonis
print(right_left3_WLU_WRU_group2.adonis)

right_left3_WLU_WRU_group2.beta.permute = right_left3_WLU_WRU_group2.perm$beta
print(right_left3_WLU_WRU_group2.beta.permute)
```

The permanova comparing left lung aspirated with whole gastric fluid to unaspirated right lung from the same animal yields a significant difference (p-value of `r right_left3_WLU_WRU_group2.adonis$aov.tab$"Pr(>F)"[1]`). 
The test for homogeneity of dispersions is insignificant (p-value of
`r right_left3_WLU_WRU_group2.beta.permute$tab$"Pr(>F)"[1]`), providing strong evidence that the aspiration treatment is altering the microbial community.

##### LefSE
```{r right_left3_WLU_WRU_group2_generate_lefse, echo=FALSE, results='hide'}
right_left3_WLU_WRU_group2_lefse.file = file.path(lefse_outdir,"right_left3_WLU_WRU_group2.tsv")
GenerateLefseOutput(right_left3_WLU_WRU_group2.pruned.ps, 
                    output_columns = c(group_var),
                    outfile = right_left3_WLU_WRU_group2_lefse.file)

Sys.setenv(LEFSE_INPUT_FILE = right_left3_WLU_WRU_group2_lefse.file)
```
...
```{r, echo=FALSE, results = 'asis'}
ShowFigure(paste0(file_path_sans_ext(right_left3_WLU_WRU_group2_lefse.file), ".png"))
```
...
The test for homogeneity of dispersions is significant at a p-value of
...
The permanova comparing right lung samples by aspiration does not have a significant difference (p-value of `r right3_SRU_WRU.adonis$aov.tab$"Pr(>F)"[1]`). 
The test for homogeneity of dispersions is also insignificant (p-value of
`r right3_SRU_WRU.beta.permute$tab$"Pr(>F)"[1]`).
...
print(right3_ASWRU_WRU.nmds.plot)
```
