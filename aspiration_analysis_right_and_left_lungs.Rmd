...
> Why are we excluding cohort #2 of WLU and WRU here? Were the cohorts run in separate batches?
...
right_left3_WLU_WRU_cohort2_lefse.res = paste0(
  file_path_sans_ext(right_left3_WLU_WRU_cohort2_lefse.file), ".res")

if(file.exists(right_left3_WLU_WRU_cohort2_lefse.res)){
...
## Drop Outliers from WRU cohort #2 
### WLU vs WRU, cohort #2, drop outliers
```{r, echo=FALSE, results = 'asis'}
nooutliers_WLU_WRU_cohort2.ps = subset_samples(bacteria.ps,
                               (((group == "WLU") & (animal >= 13)) |
                               ((group == "WRU") & 
                                  (animal %in% setdiff(seq(13,24), c(14, 15, 18, 19))))))

nooutliers_WLU_WRU_cohort2.pruned.ps = PruneTaxa(nooutliers_WLU_WRU_cohort2.ps)

## Transform to even sampling depth.
nooutliers_WLU_WRU_cohort2.even.ps = TransformCounts(nooutliers_WLU_WRU_cohort2.pruned.ps)
group_var = "group"
```
```{r, echo=FALSE, results='hide'}
sample_data(nooutliers_WLU_WRU_cohort2.ps)
```

##### NMDS
```{r, echo=FALSE, results='hide'}
nooutliers_WLU_WRU_cohort2.nmds.plot = NMDSPlot(nooutliers_WLU_WRU_cohort2.even.ps, grouping = group_var)
print(nooutliers_WLU_WRU_cohort2.nmds.plot)
```
...
##### Permanova
```{r, echo=FALSE}
nooutliers_WLU_WRU_cohort2.perm = RunPermanova(nooutliers_WLU_WRU_cohort2.even.ps, group_var)
nooutliers_WLU_WRU_cohort2.adonis = nooutliers_WLU_WRU_cohort2.perm$adonis
print(nooutliers_WLU_WRU_cohort2.adonis)

nooutliers_WLU_WRU_cohort2.beta.permute = nooutliers_WLU_WRU_cohort2.perm$beta
print(nooutliers_WLU_WRU_cohort2.beta.permute)
```

The permanova comparing left lung aspirated with whole gastric fluid to unaspirated right lung from the same animal yields a significant difference (p-value of `r nooutliers_WLU_WRU_cohort2.adonis$aov.tab$"Pr(>F)"[1]`). 
The test for homogeneity of dispersions is insignificant (p-value of
`r nooutliers_WLU_WRU_cohort2.beta.permute$tab$"Pr(>F)"[1]`), providing strong evidence that the aspiration treatment is altering the microbial community.

##### LefSE
```{r nooutliers_WLU_WRU_cohort2_generate_lefse, echo=FALSE, results='hide'}
nooutliers_WLU_WRU_cohort2_lefse.file = file.path(lefse_outdir,"nooutliers_WLU_WRU_cohort2.tsv")
GenerateLefseOutput(nooutliers_WLU_WRU_cohort2.pruned.ps, 
                    output_columns = c(group_var),
                    outfile = nooutliers_WLU_WRU_cohort2_lefse.file)

Sys.setenv(LEFSE_INPUT_FILE = nooutliers_WLU_WRU_cohort2_lefse.file)
```
...
```{r, echo=FALSE, results = 'asis', eval=FALSE}
ShowFigure(paste0(file_path_sans_ext(nooutliers_WLU_WRU_cohort2_lefse.file), ".png"))
```
Despite a significant permanova, LefSE does not find any taxa with significantly differential abundance.  Note that this is a different result than with the outliers included - in that case the permanova is also significant, but LefSE finds a differentially abundant taxon. This result with outliers excluded calls that result into question.

<!---
Confirmed: No differentially abundant features found in workspace/lefse_aspiration_left_right/nooutliers_WLU_WRU_cohort2.res
-->

### WLU vs WRU, all, drop outliers
```{r, echo=FALSE, results = 'asis'}
nooutliers_WLU_WRU_all.ps = subset_samples(bacteria.ps,
                               ((group == "WLU") |
                               ((group == "WRU") & 
                                  !(animal %in% c(14, 15, 18, 19)))))

sample_data(nooutliers_WLU_WRU_all.ps)$cohort = get_variable(nooutliers_WLU_WRU_all.ps, "animal") %>%
  divide_by_int(13) %>%
  add(1) %>% 
  factor

nooutliers_WLU_WRU_all.pruned.ps = PruneTaxa(nooutliers_WLU_WRU_all.ps)

## Transform to even sampling depth.
nooutliers_WLU_WRU_all.even.ps = TransformCounts(nooutliers_WLU_WRU_all.pruned.ps)
group_var = "group"
```
```{r, echo=FALSE, results='hide'}
sample_data(nooutliers_WLU_WRU_all.ps)
```

##### NMDS
```{r, echo=FALSE, results='hide'}
nooutliers_WLU_WRU_all.nmds.plot = NMDSPlot(nooutliers_WLU_WRU_all.even.ps, grouping = group_var)
print(nooutliers_WLU_WRU_all.nmds.plot + aes(shape=cohort))
```
...
```{r, echo=FALSE}
nooutliers_WLU_WRU_all.perm = RunPermanova(nooutliers_WLU_WRU_all.even.ps, group_var)
nooutliers_WLU_WRU_all.adonis = nooutliers_WLU_WRU_all.perm$adonis
print(nooutliers_WLU_WRU_all.adonis)

nooutliers_WLU_WRU_all.beta.permute = nooutliers_WLU_WRU_all.perm$beta
print(nooutliers_WLU_WRU_all.beta.permute)
```

The permanova comparing left lung aspirated with whole gastric fluid to unaspirated right lung from the same animal yields a significant difference (p-value of `r nooutliers_WLU_WRU_all.adonis$aov.tab$"Pr(>F)"[1]`). 
The test for homogeneity of dispersions is insignificant (p-value of
`r nooutliers_WLU_WRU_all.beta.permute$tab$"Pr(>F)"[1]`), providing strong evidence that the aspiration treatment is altering the microbial community.

##### LefSE
```{r nooutliers_WLU_WRU_all_generate_lefse, echo=FALSE, results='hide'}
nooutliers_WLU_WRU_all_lefse.file = file.path(lefse_outdir,"nooutliers_WLU_WRU_all.tsv")
GenerateLefseOutput(nooutliers_WLU_WRU_all.pruned.ps, 
                    output_columns = c(group_var,"cohort"),
                    outfile = nooutliers_WLU_WRU_all_lefse.file)

Sys.setenv(LEFSE_INPUT_FILE = nooutliers_WLU_WRU_all_lefse.file)
```
...
```{r, echo=FALSE, results = 'asis', eval=FALSE}
ShowFigure(paste0(file_path_sans_ext(nooutliers_WLU_WRU_all_lefse.file), ".png"))
```

As with the separated cohorts, combining them finds a significant difference between between lungs aspirated with whole gastric fluid and the untreated right lung from the same animal.  However, the lefse analysis fails to find taxa with significantly different abundance.

<!---
Confirmed: No differentially abundant features found in workspace/lefse_aspiration_left_right/nooutliers_WLU_WRU_all.res
-->

### Right Lung, Saline vs Whole Gastric cohort #2 (SRU vs WRU 13-24), drop outliers
```{r, echo=FALSE, results = 'asis'}
nooutliers_SRU_WRU.ps = subset_samples(bacteria.ps, 
                                    ((group == "SRU") |
                                       ((group == "WRU") & 
                                          (animal %in% setdiff(seq(13,24), c(14, 15, 18, 19))))))

nooutliers_SRU_WRU.pruned.ps = PruneTaxa(nooutliers_SRU_WRU.ps)

## Transform to even sampling depth.
nooutliers_SRU_WRU.even.ps = TransformCounts(nooutliers_SRU_WRU.pruned.ps)
group_var = "group"
```

#### NMDS
```{r, echo=FALSE, results='hide'}
nooutliers_SRU_WRU.nmds.plot = NMDSPlot(nooutliers_SRU_WRU.even.ps, grouping = group_var)
print(nooutliers_SRU_WRU.nmds.plot +geom_text(aes(label=animal),hjust=0, vjust=0))
```

Without the outliers from WRU cohort #2, the NMDS plot shows a clear separation betweeen the untreated right lungs from animals aspirated with saline vs those aspirated with whole gastric fluid.

#### Permanova
```{r, echo=FALSE}
nooutliers_SRU_WRU.perm = RunPermanova(nooutliers_SRU_WRU.even.ps, group_var)
nooutliers_SRU_WRU.adonis = nooutliers_SRU_WRU.perm$adonis
print(nooutliers_SRU_WRU.adonis)

nooutliers_SRU_WRU.beta.permute = nooutliers_SRU_WRU.perm$beta
print(nooutliers_SRU_WRU.beta.permute)
```

The permanova comparing right lung samples by left-lung aspiration does show a significant difference (p-value of `r nooutliers_SRU_WRU.adonis$aov.tab$"Pr(>F)"[1]`). 
The test for homogeneity of dispersions is insignificant (p-value of
`r nooutliers_SRU_WRU.beta.permute$tab$"Pr(>F)"[1]`).

#### LefSE
```{r, echo=FALSE, results='hide'}
nooutliers_SRU_WRU_lefse.file = file.path(lefse_outdir,"nooutliers_SRU_WRU.tsv")
GenerateLefseOutput(nooutliers_SRU_WRU.pruned.ps, 
                    output_columns = c(group_var),
                    outfile = nooutliers_SRU_WRU_lefse.file)

Sys.setenv(LEFSE_INPUT_FILE = nooutliers_SRU_WRU_lefse.file)
```
...
```{r, echo=FALSE, results = 'asis', eval=FALSE}
ShowFigure(paste0(file_path_sans_ext(nooutliers_SRU_WRU_lefse.file), ".png"))
```

Despite a significant permanova, LefSE does not find any taxa with significantly differential abundance.  This is the same as with the outliers included.

<!---
Confirmed: No differentially abundant features found in workspace/lefse_aspiration_left_right/nooutliers_SRU_WRU.res
-->

### Right Lung, Whole Gastric with and without (cohort #2) Antibiotics (WRU 13-24 vs ASWRU), drop outliers

```{r, echo=FALSE, results = 'asis'}
nooutliers_ASWRU_WRU.ps = subset_samples(bacteria.ps,
                                    ((group == "ASWRU") |
                                       ((group == "WRU") & 
                                          (animal %in% setdiff(seq(13,24), c(14, 15, 18, 19))))))

nooutliers_ASWRU_WRU.pruned.ps = PruneTaxa(nooutliers_ASWRU_WRU.ps)

## Transform to even sampling depth.
nooutliers_ASWRU_WRU.even.ps = TransformCounts(nooutliers_ASWRU_WRU.pruned.ps)
group_var = "group"
```

```{r, echo=FALSE, results='hide'}
sample_data(nooutliers_ASWRU_WRU.ps)
```

#### NMDS
```{r, echo=FALSE, results='hide'}
nooutliers_ASWRU_WRU.nmds.plot = NMDSPlot(nooutliers_ASWRU_WRU.even.ps, grouping = group_var)
print(nooutliers_ASWRU_WRU.nmds.plot +geom_text(aes(label=animal),hjust=0, vjust=0))

```

Dropping the outliers from WRU allows us to better visualize the remaining samples, the NMDS seems to indicate no clear difference between WRU and ASWRU, indicating that antibiotic treatment doesn't have a major effect on non-aspirated lungs.

#### Permanova
```{r, echo=FALSE}
nooutliers_ASWRU_WRU.perm = RunPermanova(nooutliers_ASWRU_WRU.even.ps, group_var)
nooutliers_ASWRU_WRU.adonis = nooutliers_ASWRU_WRU.perm$adonis
print(nooutliers_ASWRU_WRU.adonis)

nooutliers_ASWRU_WRU.beta.permute = nooutliers_ASWRU_WRU.perm$beta
print(nooutliers_ASWRU_WRU.beta.permute)
```

The permanova comparing right lung samples by aspiration does not have a significant difference (p-value of `r nooutliers_ASWRU_WRU.adonis$aov.tab$"Pr(>F)"[1]`). 
The test for homogeneity of dispersions is also insignificant (p-value of
`r nooutliers_ASWRU_WRU.beta.permute$tab$"Pr(>F)"[1]`).

#### LefSE
```{r, echo=FALSE, results='hide'}
nooutliers_ASWRU_WRU_lefse.file = file.path(lefse_outdir,"nooutliers_ASWRU_WRU.tsv")
GenerateLefseOutput(nooutliers_ASWRU_WRU.pruned.ps, 
                    output_columns = c(group_var),
                    outfile = nooutliers_ASWRU_WRU_lefse.file)

Sys.setenv(LEFSE_INPUT_FILE = nooutliers_ASWRU_WRU_lefse.file)
```
...
```{r, echo=FALSE, results = 'asis', eval=FALSE}
ShowFigure(paste0(file_path_sans_ext(nooutliers_ASWRU_WRU_lefse.file), ".png"))
```

As expected, due to the lack of significance in the permanova, LefSE does not find a significant difference in abundance between antibiotic treated and untreated animals in the (unaspirated) right lungs of animals whose left lungs were aspirated with whole gastric fluid.  **Note** that this is a different result than with the outliers included - in that case the permanova is still non-significant, but LefSE finds a differentially abundant taxon.  This result with outliers excluded calls that result into question.

<!---
Confirmed: No differentially abundant features found in workspace/lefse_aspiration_left_right/nooutliers_ASWRU_WRU.res
-->
