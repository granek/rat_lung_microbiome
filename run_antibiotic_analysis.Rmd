...
```{r show_all_pngs, echo = F, results = 'asis'}
...
```{r show_all_pngs2, echo = F, results = 'asis'}
lefse_dir="workspace/lefse"

all_pngs = list.files(lefse_dir, pattern = ".png$",recursive = TRUE,full.names = TRUE)
print(all_pngs)

grep("bool",all_pngs)

grep("factor",all_pngs)

```


```
for (cur_png in list.files(lefse_dir, pattern = ".*factor*png$",recursive = TRUE,full.names = TRUE)){
  md_string = paste0("![This is ", cur_png,"](", cur_png, ")\n")
  cat(md_string, fill=TRUE)
}
```
